...
export const ChildPovertyRate: React.FC<Props> = (props) => {
    const { primary, comparison } = props;

    const q1Primary = Number(
        primary.feature.properties[StateLevelVariable.Q1ChildPoverty]
    ).toFixed(1);

    const q4Primary = Number(
        primary.feature.properties[StateLevelVariable.Q4ChildPoverty]
    ).toFixed(1);

    const q1Comparison = Number(
        comparison.feature.properties[StateLevelVariable.Q1ChildPoverty]
    ).toFixed(1);

    const q4Comparison = Number(
        comparison.feature.properties[StateLevelVariable.Q4ChildPoverty]
    ).toFixed(1);
